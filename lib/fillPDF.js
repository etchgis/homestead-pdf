...
export default async function fillPDF(inputArray, dest) {
  try {
    ...
    const formPdfBytes = readFileSync("c:/node-apps/homestead-pdf/data/dr501.pdf");
    ...
    fieldMap.forEach((field) => {
      ...
      switch (fieldType) {
        ...
        case "PDFSignature":
          ...
          break;
        default:
          console.log(`Field type ${fieldType} not recognized`);
    ...
    if (dest) {
      ...
      const destination = dest || "C:/inetpub/internal/homestead-admin/applications/homestead-" + formattedDate + "-" + data?.application_serial + ".pdf";
      // const destination = dest || "./homestead-" + formattedDate + "-" + data?.application_serial + ".pdf";
      writeFileSync(destination, file);
      return destination
    }
